...
public class InterviewEmployeeControllerTest {
    ...
    public void getAllByEmployeeId() {
        ...
        ResponseEntity<List<InterviewEmployeeDTO>> response1 = doGetByInterviewIdRequest(restTemplate, interviewEmployeeMapper.fromDTO(response.getBody()).getEmployee().getId());
        Assert.assertEquals(response.getStatusCode(), HttpStatus.OK);
